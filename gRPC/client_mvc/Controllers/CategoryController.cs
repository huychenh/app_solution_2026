...
namespace client_mvc.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        private readonly ShopOnline.Api.CategoryGrpc.CategoryGrpcClient _categoryGrpcClient;

        public CategoryController(ILogger<CategoryController> logger,
            IHttpClientFactory httpClientFactory,
            IConfiguration configuration,
            ShopOnline.Api.CategoryGrpc.CategoryGrpcClient categoryGrpcClient)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                // 1. Attach the Bearer token into gRPC Metadata (Headers)
                var headers = new Metadata
                {
                    { "Authorization", $"Bearer {accessToken}" }
                };

                // 2. Call the gRPC Server using the auto-generated client instead of HttpClient
                // We pass an empty request since we want to fetch the whole list (or update your proto to support lists)
                var response = await _categoryGrpcClient.GetCategoryListAsync(new EmptyRequest(), headers);

                // 3. Map the gRPC response model to your MVC View model (CategoryReadDto)                
                var categories = response.Categories.Select(c => new CategoryReadDto
                {
                    Id = c.Id,
                    Name = c.Name,
                    Description = c.Description,
                    
                    // Check if the string is not empty before parsing to DateTime
                    CreatedDate = !string.IsNullOrEmpty(c.CreatedDate) 
                        ? DateTime.Parse(c.CreatedDate) 
                        : DateTime.MinValue,

                    // Safely handle nullable UpdatedDate
                    UpdatedDate = (DateTime)(!string.IsNullOrEmpty(c.UpdatedDate) 
                        ? DateTime.Parse(c.UpdatedDate) 
                        : (DateTime?)null),
            ...
            {
                _logger.LogWarning(rpcEx, "Unauthorized gRPC call.");
                TempData["ErrorMessage"] = "You are not authorized to view this page. Please log in again.";
                return View("Index", null);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An error occurred while fetching Categories via gRPC.");
        ...
        public async Task<IActionResult> Create(CategoryCreateViewModel model)
        {
            ...
            try
            {
                ...
                {
                    // FIXED: Using TempData to make sure the error notification shows up on the View
                ...
                {
                    // FIXED: Using TempData to make sure the error notification shows up on the View
                    TempData["ErrorMessage"] = $"Failed to create Category. Server responded with status code: {(int)response.StatusCode}";
                    return View(model);
                }

                // FIXED: Set success message to TempData before redirecting back to Index
